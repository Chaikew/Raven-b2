...
{
    public static ArrayList<Category> c4tL1st;
    
    public ClickGui() {
        ClickGui.c4tL1st = new ArrayList<Category>();
        int y = 5;
        Module.category[] values;
        for (int length = (values = Module.category.values()).length, i = 0; i < length; ++i) {
            final Module.category c = values[i];
            final Category f = new Category(c);
            f.y(y);
    ...
    public void drawScreen(final int x, final int y, final float p) {
        ...
        for (final Category chromaManager : ClickGui.c4tL1st) {
            chromaManager.rf(this.fontRendererObj);
            chromaManager.up(x, y);
            for (final ModuleDescRender m : chromaManager.gc()) {
                m.uu(x, y);
            }
        }
        this.drawPlayerMiniature(x, y, (EntityLivingBase)this.mc.thePlayer);
    }
    
    public void mouseClicked(final int x, final int y, final int mo) {
        for (final Category c4t : c4tL1st) {
    ...
    public void keyTyped(final char typedChar, final int keyCode) {
        for (final Category c4t : ClickGui.c4tL1st) {
            if (c4t.fv() && keyCode != 1 && !c4t.gc().isEmpty()) {
                for (final ModuleDescRender moduleDescRender : c4t.gc()) {
                    moduleDescRender.ky(typedChar, keyCode);
                }
            }
        }
        if (keyCode == 1) {
    ...
    private void drawPlayerMiniature(final int x, final int y, final EntityLivingBase e) {
